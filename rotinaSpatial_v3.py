...
#     volta_nivel+barra+'saidas'+barra+'dados_papaer1.csv'
...
# fig = plt.figure(figsize=(10, 5)) ###vulner_dia
# ax = fig.add_subplot()
# dados_ibge_4.boundary.plot(ax=ax,color = 'gray',label = 'Census Sectors IBGE',linewidth=0.5)

# #dados_ibge.centroid.plot(ax=ax,color = 'blue')

# plt.scatter(dados_todos_15.LONGITUDE, dados_todos_15.LATITUDE, c='red', label = 'Consumer Units',s=0.5)
...
plt.xticks(np.arange(-0.7, 8, 1))
plt.xlabel('Irregularity Occurrence', size=15)
plt.ylabel('Spatial Lag', size=15)
ax.tick_params(axis='both', which='major', labelsize=15)
plt.text(1.96, 0.51, 'HH', fontsize=15)
plt.text(1.95, -1.0, 'HL', fontsize=15)
plt.text(-0.55, 0.6, 'LH', fontsize=15)
plt.text(-0.55, -1, 'LL', fontsize=15)
...
caminho_DADOSCELESC_UCS = diretorio_atual+barra + \
    volta_nivel+barra+'entradas'+barra+'DADOSCELESC_UCS.csv'
#%%

dados_DADOSCELESC_UCS = pd.read_csv(caminho_DADOSCELESC_UCS, sep=',')
...
# #google_earth_points = list(zip(dados_DADOSCELESC_UCS['LATITUDE'], dados_DADOSCELESC_UCS['LONGITUDE'], dados_DADOSCELESC_UCS['UNI_TR_MT']))
...
splot.esda.lisa_cluster(moran_local, gdf[["PARECER2_TOTAL", "geometry"]],s=10)
...
# 2. Pass the axis to lisa_cluster
splot.esda.lisa_cluster(
    moran_local, 
    gdf[["PARECER2_TOTAL", "geometry"]], 
    s=10, 
    ax=ax
)
...
# 4. Label the latitude and longitude axes
ax.set_xlabel("Longitude", fontsize=11)
ax.set_ylabel("Latitude", fontsize=11)

# 5. Format ticks (enables latitude/longitude degree format if needed)
ax.tick_params(axis='both', which='major', labelsize=9)

# Optional: Ensure grid displays behind the plot points
...
dados_todos_15 = dados_todos_15.dropna()
...
# cod_setor = gdf_dados_todos_13['Cod_setor'].value_counts()
# numero_UC = gdf_dados_todos_13['UC'].drop_duplicates().value_counts()
# numero_TR = gdf['UNI_TR_MT'].value_counts()
