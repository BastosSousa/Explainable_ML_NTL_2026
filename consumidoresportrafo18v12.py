...
def mad_based_outlier(points, threshold=3):
  ...
  modified_z_score = 0.5645 * deviation / mad
  return modified_z_score > threshold

#%%

dados_todos_0 = dados_todos.dropna(subset=['LATITUDE', 'LONGITUDE'])

#%%
# Implement the mad_based_outlier function to check for outliers in both longitude and latitude
outliers_longitude = mad_based_outlier(dados_todos_0['LONGITUDE'])
outliers_latitude = mad_based_outlier(dados_todos_0['LATITUDE'])

outliers = outliers_longitude | outliers_latitude

#%%
dados_todos_0 = dados_todos_0.loc[~outliers]
...
# plt.scatter(dados_todos[~outliers]['LONGITUDE'], dados_todos[~outliers]['LATITUDE'], c='blue', label='Inliers')
# plt.scatter(dados_todos[outliers]['LONGITUDE'], dados_todos[outliers]['LATITUDE'], c='red', label='Outliers')
...
#%%
dados_todos_2 = dados_todos_0.copy()
...
#%%
PARECER2_TOTAL = dados_inspe_2['PARECER2'].value_counts()

Parecer_UC = dados_inspe_2['UC'].value_counts()

#%%
dados_todos_UC = dados_todos_10[['Cod_setor', 'UNI_TR_MT', 'UC', 'LATITUDE', 'LONGITUDE','PO_NOMINAL_TRAFO',
'Porc']]
...
for i in range(columns):
    ...
    dados_ibge.boundary.plot(ax=ax,color = 'grey',label = 'census areas')
    #plt.scatter(teste_2_2['x_coords'], teste_2_2['y_coords'], 10, 'r', edgecolor='w', label='CU')
    ...
    #plt.show()
    plt.title('IDW interpolation - {0}'.format(names[cnt]))
