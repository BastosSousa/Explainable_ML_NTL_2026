...
#%%
for feature in top6_feature:   # ou sua lista de features
...
clusters = KMeans(4).fit_predict(shap_values.values)
...
for group, cols in inkar_groups.items():
    ...
    group_importance[group] = np.abs(shap_values.values[:, idx]).mean()
...
for group, cols in inkar_groups.items():
    idx = [standardized_df.columns.get_loc(c) for c in cols]
    gdf_GW_SHAP[group] = abs(shap_values.values[:, idx]).mean(axis=1)
...
for group, cols in inkar_groups.items():
    ...
    gdf_GW_SHAP[group] = np.abs(shap_values.values[:, idx]).mean(axis=1)
