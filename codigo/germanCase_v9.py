...
#%%
#%% ============================================================
# BLOCO NOVO: ANÁLISE ESTATÍSTICA DE PERDAS (1 ANO)
# 1) perdas per capita
# 2) gráfico log-log
# 3) mapa espacial
# 4) perdas vs densidade energética (Zensus)
# ===============================================================
...
out_dir_stats = diretorio_atual + barra + volta_nivel + barra + 'saidas' + barra + 'figuras_perdas'
os.makedirs(out_dir_stats, exist_ok=True)

#%%

df_perdas = dados_todos.copy()

# garantir tipos numéricos
df_perdas['Summe der Netzverluste [kWh]'] = pd.to_numeric(
    df_perdas['Summe der Netzverluste [kWh]'], errors='coerce'
)

df_perdas['Einwohnerzahl'] = pd.to_numeric(
    df_perdas['Einwohnerzahl'], errors='coerce'
)

df_perdas['Preço [Ct/kWh]'] = pd.to_numeric(
    df_perdas['Preis [Ct/kWh]'], errors='coerce'
)

# evitar problemas com população 0 ou NaN
df_perdas = df_perdas.dropna(subset=['Summe der Netzverluste [kWh]'])
df_perdas = df_perdas[df_perdas['Summe der Netzverluste [kWh]'] > 0]

# criar perdas per capita
df_perdas['perdas_per_capita_kWh_hab'] = np.where(
    (df_perdas['Einwohnerzahl'].notna()) & (df_perdas['Einwohnerzahl'] > 0),
    df_perdas['Summe der Netzverluste [kWh]'] / df_perdas['Einwohnerzahl'],
    np.nan
)

# log10 para gráficos com outliers
df_perdas['log_perdas_kWh'] = np.log10(df_perdas['Summe der Netzverluste [kWh]'])
df_perdas['log_pop'] = np.where(
    (df_perdas['Einwohnerzahl'].notna()) & (df_perdas['Einwohnerzahl'] > 0),
    np.log10(df_perdas['Einwohnerzahl']),
    np.nan
)

#%%

# trazer coordenadas já geocodificadas
if 'latitude' in gdf_dados_todos.columns and 'longitude' in gdf_dados_todos.columns:
    geo_aux = gdf_dados_todos.drop(columns='geometry', errors='ignore').copy()
    geo_aux = geo_aux[['MaStR-Nr.', 'latitude', 'longitude']].drop_duplicates()
    df_perdas = df_perdas.merge(geo_aux, on='MaStR-Nr.', how='left')

#%%
# GeoDataFrame de perdas
gdf_perdas = gpd.GeoDataFrame(
    df_perdas,
    geometry=gpd.points_from_xy(df_perdas['longitude'], df_perdas['latitude']),
    crs='EPSG:4326'
)
#%%
gdf_perdas = gdf_perdas.dropna(subset=['geometry']).copy()

#%%
gdf_perdas_25832 = gdf_perdas.to_crs(epsg=25832)

#%%

df_plot_pc = df_perdas.dropna(subset=['perdas_per_capita_kWh_hab']).copy()

fig, ax = plt.subplots(figsize=(10, 6))
ax.hist(df_plot_pc['perdas_per_capita_kWh_hab'], bins=15)
ax.set_title('Distribuição de perdas per capita dos DSOs')
ax.set_xlabel('Perdas per capita [kWh/hab]')
ax.set_ylabel('Frequência')
fig.savefig(os.path.join(out_dir_stats, 'hist_perdas_per_capita.png'),
...
# ranking simples
ranking_perdas_pc = df_plot_pc[
    ['Name des Marktakteurs', 'Ort', 'Summe der Netzverluste [kWh]',
     'Einwohnerzahl', 'perdas_per_capita_kWh_hab']
].sort_values('perdas_per_capita_kWh_hab', ascending=False)

ranking_perdas_pc.to_csv(
    os.path.join(out_dir_stats, 'ranking_perdas_per_capita.csv'),
    sep=';', index=False, encoding='utf-8-sig'
)

#%%

df_plot_log = df_perdas.dropna(subset=['log_perdas_kWh', 'log_pop']).copy()

fig, ax = plt.subplots(figsize=(10, 6))
ax.scatter(df_plot_log['Einwohnerzahl'], df_plot_log['Summe der Netzverluste [kWh]'])
ax.set_xscale('log')
ax.set_yscale('log')
ax.set_title('Perdas vs População (escala log-log)')
ax.set_xlabel('População')
ax.set_ylabel('Perdas [kWh]')
fig.savefig(os.path.join(out_dir_stats, 'scatter_loglog_perdas_populacao.png'),
            dpi=300, bbox_inches='tight')
plt.show()
#plt.close(fig)
...
if len(df_plot_log) >= 2:
    coef = np.polyfit(df_plot_log['log_pop'], df_plot_log['log_perdas_kWh'], 1)
    xfit = np.linspace(df_plot_log['log_pop'].min(), df_plot_log['log_pop'].max(), 100)
    yfit = coef[0] * xfit + coef[1]

    fig, ax = plt.subplots(figsize=(10, 6))
    ax.scatter(df_plot_log['log_pop'], df_plot_log['log_perdas_kWh'])
    ax.plot(xfit, yfit)
    ax.set_title('Relação log10(perdas) vs log10(população)')
    ax.set_xlabel('log10(População)')
    ax.set_ylabel('log10(Perdas [kWh])')
    fig.savefig(os.path.join(out_dir_stats, 'scatter_log10_regressao.png'),
                dpi=300, bbox_inches='tight')
    plt.show()
    #plt.close(fig)
#%%

dados_shp_4326 = dados_shp.to_crs(epsg=4326)

# mapa de perdas absolutas
fig, ax = plt.subplots(figsize=(10, 10))
dados_shp_4326.boundary.plot(ax=ax, linewidth=0.8)
gdf_perdas.plot(
    ax=ax,
    column='Summe der Netzverluste [kWh]',
    legend=True,
    markersize=40
)
ax.set_title('Mapa dos DSOs - perdas absolutas [kWh]')
fig.savefig(os.path.join(out_dir_stats, 'mapa_perdas_absolutas.png'),
            dpi=300, bbox_inches='tight')
plt.show()
#plt.close(fig) 
#%%

# mapa de perdas per capita
gdf_perdas_pc = gdf_perdas.dropna(subset=['perdas_per_capita_kWh_hab']).copy()

fig, ax = plt.subplots(figsize=(10, 10))
dados_shp_4326.boundary.plot(ax=ax, linewidth=0.8)
gdf_perdas_pc.plot(
    ax=ax,
    column='perdas_per_capita_kWh_hab',
    legend=True,
    markersize=40
)
ax.set_title('Mapa dos DSOs - perdas per capita [kWh/hab]')
fig.savefig(os.path.join(out_dir_stats, 'mapa_perdas_per_capita.png'),
            dpi=300, bbox_inches='tight')
plt.show()
#plt.close(fig) 
...
#%% 
cols_excluir_zensus = {
    'GITTER_ID_10km', 'x_mp_10km', 'y_mp_10km', 'geometry'
}
cols_energ = [c for c in gdf.columns if c not in cols_excluir_zensus]

# manter só colunas numéricas de energieträger
cols_energ_num = []
for c in cols_energ:
    gdf[c] = pd.to_numeric(gdf[c], errors='coerce')
    if pd.api.types.is_numeric_dtype(gdf[c]):
        cols_energ_num.append(c)

# proxy de densidade energética:
# soma dos domicílios/contagens por energieträger no grid de 10km
gdf['energia_total_proxy'] = gdf[cols_energ_num].sum(axis=1, skipna=True)

# área do grid de 10x10 km = 100 km²
gdf['densidade_energetica_proxy'] = gdf['energia_total_proxy'] / 100.0

# join do DSO ao grid mais próximo do Zensus
# ambos em EPSG:25832
gdf_zensus_join = gdf[['GITTER_ID_10km', 'energia_total_proxy',
                       'densidade_energetica_proxy', 'geometry']].copy()

gdf_perdas_zensus = gpd.sjoin_nearest(
    gdf_perdas_25832,
    gdf_zensus_join,
    how='left',
...
# scatter perdas per capita vs densidade energética
df_scatter_energy = gdf_perdas_zensus.dropna(
    subset=['perdas_per_capita_kWh_hab', 'densidade_energetica_proxy']
).copy()

fig, ax = plt.subplots(figsize=(10, 6))
ax.scatter(
    df_scatter_energy['densidade_energetica_proxy'],
    df_scatter_energy['perdas_per_capita_kWh_hab']
)
ax.set_title('Perdas per capita vs densidade energética (proxy Zensus)')
ax.set_xlabel('Densidade energética proxy [contagem/ km²]')
ax.set_ylabel('Perdas per capita [kWh/hab]')
fig.savefig(os.path.join(out_dir_stats, 'scatter_perdas_pc_vs_densidade_energetica.png'),
            dpi=300, bbox_inches='tight')
plt.close(fig)

# versão log-log para reduzir influência de outliers
df_scatter_energy = df_scatter_energy[
    (df_scatter_energy['densidade_energetica_proxy'] > 0) &
    (df_scatter_energy['perdas_per_capita_kWh_hab'] > 0)
].copy()

if len(df_scatter_energy) > 0:
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.scatter(
        df_scatter_energy['densidade_energetica_proxy'],
        df_scatter_energy['perdas_per_capita_kWh_hab']
    ...
    ax.set_xscale('log')
    ax.set_yscale('log')
    ax.set_title('Perdas per capita vs densidade energética (log-log)')
    ax.set_xlabel('Densidade energética proxy [contagem/ km²]')
    ax.set_ylabel('Perdas per capita [kWh/hab]')
    fig.savefig(os.path.join(out_dir_stats, 'scatter_loglog_perdas_pc_vs_densidade_energetica.png'),
                dpi=300, bbox_inches='tight')
    plt.close(fig)

# salvar tabela final para análise posterior
cols_export = [
    'MaStR-Nr.', 'Name des Marktakteurs', 'Ort',
    'Summe der Netzverluste [kWh]', 'Einwohnerzahl',
    'perdas_per_capita_kWh_hab', 'energia_total_proxy',
    'densidade_energetica_proxy', 'dist_to_grid_m'
]

cols_export = [c for c in cols_export if c in gdf_perdas_zensus.columns]

gdf_perdas_zensus[cols_export].to_csv(
    os.path.join(out_dir_stats, 'base_perdas_com_zensus.csv'),
    sep=';', index=False, encoding='utf-8-sig'
)

print('Figuras e tabelas salvas em:', out_dir_stats)
