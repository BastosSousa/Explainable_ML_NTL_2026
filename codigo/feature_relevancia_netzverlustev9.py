...
ax.set_xlabel(r"Observed Energy Loss [kWh]")
ax.set_ylabel(r"Predicted Energy Loss [kWh]")
...
#%% ================================
# NORMALIZED SHAP SUMMARY PLOT
# =================================
# normalize SHAP values
shap_values_norm = (
    shap_values.values /
    np.abs(shap_values.values).max()
)

# optional:
# abbreviated feature names for better visualization
feature_names_short = [
    col[:25] for col in standardized_df.columns
]
...
# axis labels
ax.set_xlabel("Normalized SHAP Value", fontsize=24)
...
# Colorbar axis (usually the second axis)
if len(fig.axes) > 1:
    cbar_ax = fig.axes[-1]
...
#%% ==========================================
# FIGURE - GW-SHAP HETEROGENEITY + MAPS
# ===========================================
...
inkar_translation = {
    "arbeitslosigkeit_allgemein": "Unemployment",
    "arbeitslosigkeit_struktur": "Employment",
    "wirtschaft": "Economy",
...
for group in inkar_groups.keys():
    ...
    values = gdf_GW_SHAP[group + "_gw"]

    gw_variability[group] = (
        values.std()
        /
        values.mean()
    )
...
gw_variability = (
    pd.Series(gw_variability)
    .sort_values(ascending=False)
)

gw_variability.index = [
    inkar_translation.get(i, i)
    for i in gw_variability.index
]
...
maps_to_plot = [
    "demografie_gw",
    "arbeitslosigkeit_allgemein_gw",
    "wirtschaft_gw"
]

vmin = min(
    gdf_GW_SHAP[m].min()
    for m in maps_to_plot
)
...
bars = ax.bar(
    gw_variability.index,
    gw_variability.values
)

ax.set_title("(a) Spatial Heterogeneity")
ax.set_ylabel("CV")
...
ax.set_title("(b) Demographic Structure")
...
gdf_GW_SHAP.plot(
    column="arbeitslosigkeit_allgemein_gw",
    cmap="viridis",
    vmin=vmin,
    vmax=vmax,
    ax=ax,
...
ax.set_title("(c) Unemployment Rate")
...
ax.set_title("(d) Economic Indicators")
...
cbar.set_label("GW-SHAP Importance")

plt.tight_layout()

plt.savefig(
    "GW_SHAP_Heterogeneity.pdf",
    bbox_inches="tight"
)
