...
y = result[target_col].values / 1e6
...
#%%
groups_to_plot = ["demografie", "arbeitslosigkeit_allgemein", "wirtschaft"]
...
ax_bar.set_title("Global Importance of INKAR Groups (GW-SHAP)")
...
for i, group in enumerate(groups_to_plot):
    ...
    title = inkar_translation.get(group, group)
    ax.set_title(title)
...
plt.title("Observed vs Predicted Network Losses")
...
#%%
gdf_result["residuals"] = residuals
...
ax.set_title("Spatial Distribution of Model Residuals")
...
# Traduz as colunas do DataFrame para passar ao SHAP
translated_feature_names = [
    translation_dict.get(col, col) for col in standardized_df.columns
]

# ---------------------------------------------------------
# 2. CONVERTER ESCALA DOS VALORES SHAP (Ajuste 1)
# Opcional: Dividir os SHAP values por 1e6 faz com que o eixo X
# fique na escala de GWh (se y estiver originalmente em kWh)
# ---------------------------------------------------------
scale_factor = 1e6  # Converte de kWh para GWh (ou suaviza a notação científica)

shap_values_scaled = shap_values / scale_factor
gw_shap_scaled = gw_shap / scale_factor

# Crear os objetos Explanation com nomes traduzidos e valores reescalados
exp_global = shap.Explanation(
    values=shap_values_scaled,
    data=X,
    feature_names=translated_feature_names,
)

exp_gw = shap.Explanation(
    values=gw_shap_scaled, data=X, feature_names=translated_feature_names
)

# ---------------------------------------------------------
# 3. RENDERIZAR FIGURA FINAL PARA A TESE
# ---------------------------------------------------------
plt.rcParams.update(
    {
        "font.family": "serif",
...
axes[1].set_title("(b) GeoSHAP Beeswarm (Spatially Weighted)", fontsize=13, pad=12)
axes[1].set_xlabel("GeoSHAP value (Local Spatial Impact [GWh])")

plt.tight_layout()

# Salvar figura para inserção no LaTeX/Word
plt.savefig(
    "figura_thesis_shap_vs_geoshap.png",
    dpi=300,
    bbox_inches="tight",
)
plt.show()