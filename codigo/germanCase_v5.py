...
#%%
dados_todos = dados_todos.rename(columns={'Ort': 'GEN'})
...
#%%
selec =['GEN','OBJECTID', 'STG', 'ADE', 'AGS', 'TGS', 'IBZ', 'BEZ', 'NBD', 'TYP', 'NLS', 'SDV', 'SDV_AGS', 'SDV_TGS', 'EWZ', 
        'KFL', 'AGS_ST', 'SHAPE_Leng', 'SHAPE_Area', 'geometry', 'y_coords', 'x_coords']


teste_2 = df_limpo[df_limpo.columns.intersection(selec)].reset_index()

#%%
teste_2 = pd.concat([teste_2, resultado], axis=1)

#%%
df_merged_2 = pd.merge(dados_todos, teste_2, on='GEN', how='left')

#%%
# Separate rows containing any NaN values
df_nan_2 = df_merged_2[df_merged_2['AGS'].isna()]

#%%

#df_merged_2 = df_merged_2.dropna(subset=["Summe der Netzverluste [kWh]"])
...
df_nan_3["x"], df_nan_3["y"] = transformer.transform(df_nan_3["longitude"].values, df_nan_3["latitude"].values)
#%%
df_nan_3 = df_nan_3.drop(columns=['longitude', 'latitude'])

# print(df.columns.tolist())

#%%

df_nan_3 = df_nan_3[['MaStR-Nr.', 'Name des Marktakteurs', 'Marktfunktion', 'Marktrollen', 'Bundesland', 'Postleitzahl', 
                'GEN', 'Straße', 'Hausnummer', 'Land', 'Registrierungsdatum', 'Datum der letzten Aktualisierung', 'ACER-Code', 
                'Geschlossenes Verteilernetz', 'Tätigkeitsstatus', 'Tätigkeitsbeginn', 'Tätigkeitsende', 'Summe der Netzverluste [kWh]', 
                'Preis [Ct/kWh]', 'Anzahl der Entnahmestellen jeweils für alle Netz', 'Einwohnerzahl', 'index', 'OBJECTID', 'STG', 'ADE', 
                'AGS', 'TGS', 'IBZ', 'BEZ', 'NBD', 'TYP', 'NLS', 'SDV', 'SDV_AGS', 'SDV_TGS', 'EWZ', 'KFL', 'AGS_ST', 'SHAPE_Leng', 'SHAPE_Area', 
                'geometry','x', 'y']].reset_index()


#%%
unknown_points_2 = df_nan_3[['x', 'y']].values
...
#%%
teste_3 = pd.concat([df_nan_3, resultado], axis=1)

#%%

df_clean = df_merged_2.dropna(subset=['AGS'])

#%%
# Concatenar verticalmente
df_merged_3 = pd.concat([df_clean, teste_3], ignore_index=True)

#%%

cols_to_drop = ['index', 'OBJECTID', 'STG', 'ADE', 'AGS', 'TGS', 'IBZ', 'BEZ', 'NBD', 'TYP', 'NLS', 'SDV', 'SDV_AGS', 
                                      'SDV_TGS', 'EWZ', 'KFL', 'AGS_ST', 'SHAPE_Leng', 'SHAPE_Area', 'geometry','level_0']


df_merged_4 = df_merged_3.drop(columns=cols_to_drop)
