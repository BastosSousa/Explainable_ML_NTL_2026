...
dados_todos['estado'] = 'Sachsen-Anhalt'
dados_todos_L['estado'] = 'Lower Saxony'
...
# %%
# ============================================================
# ANÁLISE DE CORRELAÇÃO ESPACIAL AVANÇADA
# Score de relevância por região (correlação com losses/preços)
# ============================================================

# --- 1. Definir colunas-alvo (losses e preços) ---
# Ajuste os nomes conforme suas colunas reais no dados_combinados
target_cols = [col for col in dados_combinados.columns
               if any(k in col.lower() for k in ['loss', 'price', 'preis', 'verlust', 'cost'])]
#%%
print("Colunas-alvo encontradas:", target_cols)

# --- 2. Merge espacial: juntar resultado_5 com dados DSO georreferenciados ---
...
#%%
# Associa cada ponto DSO à região (GEN) mais próxima via sjoin
gdf_dso_joined = gpd.sjoin_nearest(
    gdf_dso,
    resultado_5[['GEN', 'geometry']],
    how='left',
    distance_col='dist_m'
)
#%%
# garantir numérico
gdf_dso_joined[target_cols] = (
    gdf_dso_joined[target_cols]
    .replace(',', '.', regex=True)
...
#%%
# manter apenas linhas com dados válidos
gdf_dso_valid = gdf_dso_joined.dropna(subset=target_cols, how='all').copy()

# regiões válidas
gen_validos = gdf_dso_valid['GEN'].unique()

#%%
# --- 3. Agregar targets por região ---
target_por_regiao = (
    gdf_dso_joined
    .groupby('GEN')[target_cols]
    .mean()
    .reset_index()
)

#%%
# --- 4. Merge com as variáveis interpoladas por região ---
df_analise = resultado_5.merge(target_por_regiao, on='GEN', how='inner')

feature_cols = [c for c in resultado_5.columns
                if c not in ['GEN', 'geometry', 'latitude', 'longitude']]
#%%
# --- 5. Correlação de Pearson: cada feature vs. cada target ---
corr_records = []
for feat in feature_cols:
    for tgt in target_cols:
        subset = df_analise[[feat, tgt]].dropna()
        if len(subset) < 5:
            continue
        r = subset[feat].corr(subset[tgt])
        corr_records.append({'feature': feat, 'target': tgt, 'pearson_r': r})

df_corr = pd.DataFrame(corr_records)
df_corr['abs_r'] = df_corr['pearson_r'].abs()

# Score agregado: média do |r| de cada feature em todos os targets
df_score = (
    df_corr
    .groupby('feature')['abs_r']
    .mean()
    .reset_index()
...
print("\nTop 5 variáveis mais correlacionadas com losses/preços:")
print(df_score.head(5).to_string(index=False))

#%%
df_score.to_csv(out_dir + barra + 'feature_scores_correlacao.csv',
                sep=';', index=False, encoding='utf-8-sig')

#%%
# --- 6. Correlação Espacial de Moran (I de Moran Local) por feature top ---
top_features = df_score.head(10)['feature'].tolist()

resultado_5_proj = resultado_5.to_crs(epsg=25832)
w = weights.Queen.from_dataframe(resultado_5_proj, silence_warnings=True)
w.transform = 'r'

moran_records = []
for feat in top_features:
    y = resultado_5_proj[feat].fillna(resultado_5_proj[feat].median()).values
    mi = Moran(y, w)
    ...
    print(f"  {feat}: I={mi.I:.4f}, p={mi.p_sim:.4f}")

df_moran = pd.DataFrame(moran_records).sort_values('moran_I', ascending=False)
df_moran.to_csv(out_dir + barra + 'moran_top_features.csv',
                sep=';', index=False, encoding='utf-8-sig')

#%%
# --- 7. Score composto final (correlação + autocorrelação espacial) ---
df_final_score = df_score.merge(df_moran[['feature', 'moran_I', 'p_value']], on='feature', how='left')

# Ranking independente
df_final_score['rank_corr'] = df_final_score['mean_abs_r'].rank(ascending=False)
df_final_score['rank_moran'] = df_final_score['moran_I'].rank(ascending=False)

# Score determinístico sem peso
df_final_score['score_final'] = (
    df_final_score['rank_corr'] +
    df_final_score['rank_moran']
)

df_final_score = df_final_score.sort_values('score_final')
#%%

print("\n--- Score Final por Feature ---")
print(df_final_score[['feature', 'mean_abs_r', 'moran_I', 'score_final']].head(15).to_string(index=False))

df_final_score.to_csv(out_dir + barra + 'score_final_features.csv',
                      sep=';', index=False, encoding='utf-8-sig')

#%%
# --- 8. Visualização: heatmap de correlação top features x targets ---

pivot_heatmap = df_corr.pivot(index='feature', columns='target', values='pearson_r')
pivot_heatmap = pivot_heatmap.loc[df_score.head(10)['feature']]  # top 20

fig, ax = plt.subplots(figsize=(max(6, len(target_cols) * 2), 10))
sns.heatmap(
    pivot_heatmap,
    annot=True, fmt='.2f',
...
ax.set_title('Correlação: Features × Losses/Preços (Top 20)', fontsize=13)
plt.tight_layout()
plt.savefig(fig_dir + barra + 'heatmap_correlacao_features_targets.png', dpi=300, bbox_inches='tight')
plt.close()

print("\nAnálise espacial concluída. Outputs salvos em:", out_dir)
#%%
...
# filtrar apenas cidades com DSO
gdf_poligonos = dados_shp[dados_shp[col_nome].isin(gen_validos)].copy()

# merge
gdf_final = gdf_poligonos.merge(
    df_regiao,
    left_on=col_nome,
    right_on='GEN',
    how='left'
...
estado_por_regiao = (
    gdf_dso_valid.groupby('GEN')['estado']
    .first()
    .reset_index()
)

gdf_final = gdf_final.merge(estado_por_regiao, on='GEN', how='left')
#%%
