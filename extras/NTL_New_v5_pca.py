...
df_clust_pca_kmeans.columns.values[-4:] = ["comp1", "comp2", "comp3", "comp4"]
...
df_clust_pca_kmeans["segment"] = df_clust_pca_kmeans["segment_kmeans_pca"].map({0:"Cluster 1", 1:"Cluster 2", 2:"Cluster 3",3:"Cluster 4"})

#%%
df1 = pd.DataFrame(scores_pca)
import seaborn as sns
df1['segment_kmeans_pca'] = df_clust_pca_kmeans['segment_kmeans_pca']
sns.pairplot(df1[0:], hue='segment_kmeans_pca')
...
# for i in range(len(df_clust_pca_kmeans)):

# sns.scatterplot( x = "comp1",y="comp4", hue = "segment", data = df_clust_pca_kmeans)
...
df_umap_kmeans_2 = df_clust_pca_kmeans[['w_PARECER2_TOTAL', 'pop_count_local_moran',
                                   'pop_count_local_moran_p_sim','h3_cell','segment_kmeans_pca','PARECER2_TOTAL','Perdas']]
...
#%%
df2_data = df2[['comp1','comp2','comp3','comp4']].values
...
df3_data.drop(columns = ['LATITUDE', 'LONGITUDE','geometry','pop_count_local_moran','comp1', 'comp2','comp3','comp4',
                         'segment_kmeans_pca', 'segment', 'target','pop_count_local_moran_p_sim'], inplace = True)
...
cluster_df = pd.DataFrame(df2_data,columns =df2[['comp1','comp2','comp3','comp4']].columns)
...
plt.scatter(df2_data[cluster_vec==0, 1],df2_data[cluster_vec==0, 3], s=100, c='red', label ='Cluster 1')
plt.scatter(df2_data[cluster_vec==1, 1], df2_data[cluster_vec==1, 3], s=100, c='blue', label ='Cluster 2')
plt.scatter(df2_data[cluster_vec==2, 1], df2_data[cluster_vec==2, 3], s=100, c='green', label ='Cluster 3')
plt.xlabel('comp2')
plt.ylabel('comp4')
plt.legend()
plt.show()
...
plt.xlabel('comp1',**csfont,fontsize=18)
plt.ylabel('comp2',**csfont,fontsize=18)
plt.legend()
plt.show()
...
# shape_bairros_2 = dado_shape_FLO_4[['cd_uep','nm_uep','cd_bairro', 'cd_regiao','geometry']]
...
# shape_UCs = gdf.sjoin(shape_bairros_2, how="left")
...
# shape_bairros_3 = shape_bairros_3.to_crs('EPSG:4326')

# #%%
# shape_UCs_merge = pd.merge(shape_bairros_2, shape_UCs, on='cd_uep')
# shape_UCs_merge = shape_UCs_merge.set_geometry("geometry_x")
...
# #     lat = shape_UCs_merge.at[i, 'LATITUDE']  #latitude
# #     lng = shape_UCs_merge.at[i, 'LONGITUDE']  #longitude
# #     popup = '<br>'+'<a href="https://www.google.com/maps?layer=c&cbll=' + str(shape_UCs_merge.at[i, 'LATITUDE']) + ',' + str(shape_UCs_merge.at[i, 'LONGITUDE']) + '" target="blank">GOOGLE STREET VIEW</a>'
# #     cf_marker = folium.Marker(location=[lat,lng], popup=popup, icon = folium.Icon(color="blue", icon="remove-sign"))
# #     cf_cluster.add_child(cf_marker).add_to(map_h) 


# folium.LayerControl().add_to(map_h)

# map_h.save(r'C:\Pos\Run-py-Figures\MapaSep2.html')