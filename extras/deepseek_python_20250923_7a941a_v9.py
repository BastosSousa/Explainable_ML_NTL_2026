...
class RiskFuzzyClassifierWithTrapezoid:
    ...
    def preparar_dados(self, df):
        ...
        # Definir fronteiras fuzzy dinamicamente pelos quantis
        q20, q50, q80 = df_norm["Risk_score_new"].quantile([0.20, 0.50, 0.75])
...
def clip_and_rescale(df, cols=None, lower_q=0.20, upper_q=0.75):
    """
    Remove outliers via clipping entre os quantis [lower_q, upper_q]
    e reescala cada coluna para 0–1.
    """
    df_clean = df.copy()
    ...
    for col in cols:
        if col == 'Perdas_norm' or col == 'Parecer_norm':
            # plt.figure()
            # sns.boxplot(df[col])
            low, high = df_clean[col].quantile([lower_q, upper_q])
            df_clean[col] = np.clip(df_clean[col], low, high)
            # normalizar entre 0–1 novamente
            df_clean[col] = (df_clean[col] - df_clean[col].min()) / (df_clean[col].max() - df_clean[col].min())
    
    return df_clean

dados_clipped = clip_and_rescale(df, cols=["Perdas_norm", "Parecer_norm", "umap_1_scaled", "umap_2_scaled"])

#%%
df_norm = classificador.preparar_dados(dados_clipped)
#%%
