...
for i,row in df.iterrows():
    lat = df.at[i, 'LATITUDE_x']  #latitude
    lng = df.at[i, 'LONGITUDE_x']  #longitude
    popup = '<br>'+'<a href="https://www.google.com/maps?layer=c&cbll=' + str(df.at[i, 'LATITUDE_x']) + ',' + str(df.at[i, 'LONGITUDE_x']) + '" target="blank">GOOGLE STREET VIEW</a>'
...
gdf = gpd.GeoDataFrame(df, geometry=gpd.points_from_xy(df.LONGITUDE_x, df.LATITUDE_x), crs='EPSG:4326')
...
def geo_to_h3(row):
  return h3.latlng_to_cell(lat=row.LATITUDE_x,lng=row.LONGITUDE_x,res = res)
...
final_table_columns = ['Cod_setor', 'UC', 'Bairro', 'CD_ALIMENTADOR','PARECER2',
...
gdf_to_normalize.drop(columns = ['LATITUDE_x', 'LONGITUDE_x','geometry', 'pop_count_local_moran','h3_cell','UNI_TR_MT',], inplace = True)
...
df_umap_kmeans_2 = df_umap_kmeans[['w_PARECER2_TOTAL', 'pop_count_local_moran',
                                   'pop_count_local_moran_p_sim','h3_cell','segment_kmeans_umap']]
...
#%%
df2_data = df2.iloc[:, 99:101].values
...
df3_data.drop(columns = ['LATITUDE_x', 'LONGITUDE_x','geometry', 'UNI_TR_MT','UNI_TR_MT LABEL','pop_count_local_moran','comp1', 'comp2','segment_kmeans_umap', 'segment', 'target'], inplace = True)
...
cluster_df = pd.DataFrame(df2_data,columns = df2.iloc[:, 99:101].columns)
