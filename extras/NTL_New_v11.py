...
for i, row_i in shap_df.iterrows():
    cell_i = row_i["h3_cell"]

    neighbors = shap_df.copy()
    ...
    weighted = (
        neighbors[X_test.columns]
        .multiply(neighbors["weight"], axis=0)
        .sum()
        / neighbors["weight"].sum()
    )

    gw_shap.append(weighted)

gw_shap_df = pd.DataFrame(gw_shap)
gw_shap_df["h3_cell"] = shap_df["h3_cell"].values
...
m = gw_gdf.explore(
    column=gw_shap_df.columns[0],
    cmap="RdBu",
    legend=True,
    tiles="CartoDB positron"
)

m.save(r'C:\Pos\Run-py-Figures\gw_gdf.html')

#%%
mean_abs_shap = gw_shap_df.drop(columns=["h3_cell"]).abs().mean().sort_values(ascending=False)
importance = mean_abs_shap.sort_values(ascending=True)

plt.figure(figsize=(8, 6))
sns.barplot(x=importance.values, y=importance.index)

plt.title("Global SHAP Importance (XGBoost)")
...
gw_gdf[top_feature] = gw_shap_df[top_feature].values
...
ax.set_title(f"GW-SHAP Spatial Effect: {top_feature}")
ax.axis("on")
...
gw_gdf[second_feature] = gw_shap_df[second_feature].values

fig, ax = plt.subplots(1, 1, figsize=(10, 10))

gw_gdf.plot(
    column=second_feature,
    cmap="RdBu",
    legend=True,
    ax=ax
)

ax.set_title(f"GW-SHAP Spatial Effect: {second_feature}")
ax.axis("on")
