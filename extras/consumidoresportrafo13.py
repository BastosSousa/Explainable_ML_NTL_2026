...
# %%
dados_todos_fisca = dados_todos[['UC', 'Situação UC', 'LATITUDE', 'LONGITUDE']]
dados_fisca_2 = pd.merge(dados_fisca, dados_todos_fisca, how='left', on='UC')
...
#%%
# end = time.time()
# fmtend = time.gmtime(end)
# strfend = time.strftime("%D %T", fmtend)
...
dados_ibge_artigo = dados_ibge_dom1[['aguageral', 'aguadepoço','banheiroexclusivo','ban_esgoto','ban_outros']]
...
dados_ibge_artigo_2['aguageral'] = np.arcsinh(dados_ibge_artigo['aguageral'].values) 

dados_ibge_artigo_2['aguadepoço'] = np.arcsinh(dados_ibge_artigo['aguadepoço'].values) 

dados_ibge_artigo_2['banheiroexclusivo'] = np.arcsinh(dados_ibge_artigo['banheiroexclusivo'].values) 
