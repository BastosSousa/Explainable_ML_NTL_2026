...
feature_cols = [
    ...
    if col not in ["geometry", "h3_cell", target]
]
...
# adicionar shap values ao dataframe de teste
...
gdf_shap = pd.concat([
    gdf[['h3_cell']].reset_index(drop=True),
    shap_df
], axis=1)


def h3_kernel_weight(c1, c2, k=2):
    d = h3.grid_distance(c1, c2)
    if d is None:
        return 0
...
for i, row_i in gdf_shap.iterrows():
    cell_i = row_i['h3_cell']

    neighbors = gdf_shap.copy()
    neighbors['weight'] = neighbors['h3_cell'].apply(
        lambda c: h3_kernel_weight(cell_i, c, k=2)
    )

    weighted_shap = (
        neighbors[X.columns]
        .multiply(neighbors['weight'], axis=0)
        .sum()
        / neighbors['weight'].sum()
    )

    gw_shap.append(weighted_shap)


gw_shap_df = pd.DataFrame(gw_shap)
gw_shap_df['h3_cell'] = gdf_shap['h3_cell']

gw_shap_gdf = gpd.GeoDataFrame(
    gw_shap_df,
    geometry=gw_shap_df['h3_cell'].apply(cell_to_shapely),
    crs=4326
)

#%%
gw_shap_gdf.explore(
    column=gw_shap_df.columns[0],  # primeira feature SHAP
    cmap="RdBu",
    legend=True
)
m.save(r"C:\Pos\Run-py-Figures\gw_shap_gdf_map.html")

#%%
# shap_df["h3_cell"] = model_df.loc[X_test.index, "h3_cell"].values
# shap_df["prediction"] = y_pred
# shap_df["real"] = y_test.values

# # agregação espacial: impacto médio por célula
# geo_shap = shap_df.groupby("h3_cell").mean(numeric_only=True).reset_index()

# #%%

# geo_shap["geometry"] = geo_shap["h3_cell"].apply(
#     lambda x: cell_to_shapely(x)
# )

# gdf_geo_shap = gpd.GeoDataFrame(geo_shap, geometry="geometry", crs=4326)

# #%%

# m = gdf_geo_shap.explore(
#     column="prediction",
...
# m.save(r"C:\Pos\Run-py-Figures\geoshap_map.html")
