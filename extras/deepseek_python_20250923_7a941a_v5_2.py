...
class RiskFuzzyClassifierWithTrapezoid:
    ...
    def classificar_com_score(self):
        """
        Classifica cada ponto com base no Risk_score 1D e salva em self.resultados.
        """
        resultados = []
        for _, row in self.df_norm.iterrows():
            ...
            pertinencias = {
                "Low Risk": self.trapezio_1d(x, 0, 0.0, 0.3, 0.5),
                "Medium Risk": self.trapezio_1d(x, 0.3, 0.5, 0.5, 0.7),
                "High Risk": self.trapezio_1d(x, 0.5, 0.7, 1.0, 1.0)
            }
...
#%%
df_proc = classificador.preparar_dados(df_norm)
#%%
