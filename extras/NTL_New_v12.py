...
#%%
winner_coords = np.array([som.winner(x) for x in X_som])
som_cluster = np.ravel_multi_index(winner_coords.T, (15, 15))

#%%%

final_df = pd.DataFrame({
    "h3_cell": h3_train.values,
    "perdas_real": y_train.values,
    "som_cluster": som_cluster
})

# previsão do modelo
final_df["perdas_pred"] = xgb_model.predict(X_train)

# risco médio por célula H3
h3_risk = final_df.groupby("h3_cell").agg({
    "perdas_pred": "mean",
